...
> **Public release:** this is a curated **code + benchmark** release. The single-center patient feature table for the AIS worked example is **not included in this snapshot**; its outputs ship as the cached `figures/AIS_forensic.*` and the aggregate `results/ais_forensic.json`, `results/permutation_ICC.json`, `results/baselines_ICC.csv` and `results/ais_toolkit.json`. The load-bearing evidence, the 50-cohort benchmark, reproduces fully from public data via `radMLBench`.
...
## Key findings (all regenerated by this code; see `src/paper_numbers.py`)
...
python src/reproduce.py --fresh          # full rebuild from an empty results/ (hours)
...
```
All randomness is seeded (master seed 20260625). Without the AIS patient table, `reproduce.py` skips the three steps that need it (`permutation_test.py`, `ais_forensic.py`, `baselines.py`) and the downstream steps read their cached outputs in `results/`.
...
## Data & scope
- **The AIS cohort** feature table is **not included in this snapshot**. It is a derived feature table only (original images/masks/extraction code and clinical records are lost), collected under institutional review board approval with consent waived for retrospective analysis; institution and protocol number are withheld during double-blind review. Any released version carries surrogate case identifiers in place of the original imaging accession numbers, with no images, masks, or free-text records. Known defects: 183 rows but 180 unique source identifiers, with three pairs carrying opposite labels (one pair identical in all 1004 features); `src/ais_forensic.py` reports a sensitivity analysis excluding them. Outcome `Categories`: 1=good, 0=poor (positive=poor). Its 1004 features were **ICC (intra-class correlation) reliability-filtered**, which is why several artifacts carry `ICC` in their names; the paper names the cohort **"the AIS cohort"** so that it is not confused with that filtering step.
